...
function getTemporaryFileError() {
	...
	if ($error == 0)
		return "no error";
	if ($error == 1)
		return "Le fichier exc&egrave;de le poids autoris� par la directive upload_max_filesize";
	if ($error == 2)
		return "Le fichier exc&egrave;de le poids autoris� par le champ MAX_FILE_SIZE s'il a �t� donn�";
	if ($error == 3)
		return "Le fichier n'a �t� upload� que partiellement";
	if ($error == 4)
